...
@section('content')
    ...
    <div class="content-wrapper">
        <section class="content-header">
            <div class="row">
                <div class="col-md-6">
                    <h1><i class="fa fa-university"></i> إلغاء مستحقات مندوبي التسويق</h1>
                </div>
                                    ...
                                    <td class="text-center table_actions">
                                            @if(auth()->user()->can('sellers_payments_accept_deny_yes'))
                                                @if($seller_payment->status == 1 && $seller_payment->trans_status == 1)
                                                    <button type="button" onclick="cancelSellerPaymentTransfer('{{ route('dashboard.cancel_sellers_payments.cancel_seller_payment_transfer', $seller_payment->id) }}')" class="btn btn-warning btn-sm"><i class="fa fa-reply"></i> إلغاء الصرف</button>
                                                @endif
                                            @endif
