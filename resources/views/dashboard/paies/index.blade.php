...
@section('content')
    ...
    <div class="content-wrapper">
        <input type="hidden" id="route_url" class="form-control" value="{{ url('/dashboard/paies/setDtSearchSession') }}">
        ...
        <section class="content-header">
            <div class="row">
                ...
                <div class="col-md-6 text-left">
                    <div class="btn-group">
                        <a onclick="previewPaies(event, this)" href="{{ route('dashboard.paies.preview', $paper->id) }}" class="btn btn-default" target="_blank"><i class="fa fa-print"></i> طباعة</a>
                        ...
                        <ul class="dropdown-menu">
                            @foreach($papers as $paper)
                                <li><a onclick="previewPaies(event, this)" href="{{ route('dashboard.paies.preview', ['paper_id' => $paper->id]) }}" target="_blank"><i class="fa fa-file-o"></i> {{ $paper->paper_name }}</a></li>
                            @endforeach
            ...
            <div class="box box-primary">
                <div class="box-body">
                    <div class="table-responsive">
                        <table id="example" class="table table-hover paie_table paies_table">
                            <thead>
                            <tr>
                                ...
                                @foreach($paies as $paie)
                                    ...
                                    <tr salary_id="{{ $paie->id }}">
                                        ...
                                        <td class="text-center table_actions">
                                            ...
                                            @elseif($paie->status == -1)
                                                @if(auth()->user()->can('paies_update'))
                                                <a href="{{ route('dashboard.paies.edit', $paie->id) }}" class="btn btn-warning"><i class="fa fa-edit"></i></a>
                                                @endif
                                            @endif
                                            @if(auth()->user()->can('paies_delete'))
                                            <button type="button" onclick="deleteSalary('{{ route('dashboard.paies.delete_salary', $paie->id) }}')" class="btn btn-danger"><i class="fa fa-times"></i></button>
                                            @endif

                                            @if(auth()->user()->can('paies_accept_deny_yes'))
                                                @if($paie->status == 1 && $paie->trans_status == 1)
                                                    <button type="button" onclick="cancelSalaryTransfer('{{ route('dashboard.paies.cancel_salary_transfer', $paie->id) }}')" class="btn btn-warning btn-sm"><i class="fa fa-reply"></i> إلغاء الصرف</button>

                                                @elseif($paie->status == 1 && $paie->trans_status == 0)
                                                    <button type="button" onclick="cancelSalaryAccept('{{ route('dashboard.paies.cancel_salary_accept', $paie->id) }}')" class="btn btn-primary btn-sm"><i class="fa fa-reply"></i> إلغاء التعميد</button>
                                                @endif
                                            @endif
                                        </td>
                                        <td>{{ $paie->paie_dt }}</td>
